        ...
        PublicTutorialService.db
            ...
            .then(resp => {
                ...
                PublicTutorialService.getPublicTutById(
                    "tutorials",
                    new ObjectId(lastAdded._id)
                ).then(response => {
                    expect(response).not.toEqual(null);
                    expect(response?.content).toHaveLength(2);
                    expect(response?.content[0].sectionTitle).toEqual(
                        "Section 1"
                    );
                    expect(response?.content[0].sectionBody).toEqual(
                        "<p>So this is my section, I'm talking crap here..."
                    );
                    expect(response?.content[1].sectionTitle).toEqual(
                        "Section 2"
                    );
                    expect(response?.content[1].sectionBody).toEqual(
                        "more crap"
                    );
