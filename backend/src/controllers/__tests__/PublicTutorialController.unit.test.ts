...
describe("PublicTutorialController", () => {
    ...
    beforeAll(async () => {
        ...
        PublicTutorialService.db = await connection.db(
        ...
        await PublicTutorialService.db.collection("tutorials").insertOne({
            name: "another name",
            html:
                "somestuff that won't be considered     <p>--## Section 1</p>  <p>So this is my section, I'm talking crap here... <p>--## Section 2</p> more crap",
            markdown: "",
            category: "javascript",
    ...
    it("/public/tutorials/{tutid} should return a 204 response", (done) => {
        new App({ path: ".test.env" }).setupServer().then((app) => {
            const agent = supertest(app);
            agent
                .get(`/public/tutorials/000000000000000000000000`)
                .expect(204)
                .end(done);
        });
    });
    ...
    it("/public/tutorials/cards/{category} should return the parsed tutorial, with a 200 response", (done) => {
        new App({ path: ".test.env" }).setupServer().then((app) => {
            const agent = supertest(app);
            agent
                .get(`/public/tutorials/cards/javascripT`)
                .expect(200)
                .expect((response) => {
    ...
    it("/public/tutorials/cards/{category} should return 404, because the given category does not exist", (done) => {
        new App({ path: ".test.env" }).setupServer().then((app) => {
            ...
            agent
                .get(
                    `/public/tutorials/cards/javascripT-a-none-existent-category`
                )
                .expect(404)
                .end(done);
        });
    ...
    it("/public/tutorials/cards/{category}?offset=*&outset=* should return 400, because the given values aren't integers", (done) => {
        new App({ path: ".test.env" }).setupServer().then((app) => {
            ...
            agent
                .get(
                    `/public/tutorials/cards/javascripT?offset=bob&outset=harry`
                )
                .expect(400)
                .end(done);
        });
    ...
    it("public/tutorials/categories/query?offset=*&outset=* should return 400, because the given values aren't integers", (done) => {
        new App({ path: ".test.env" }).setupServer().then((app) => {
            ...
            agent
                .get(
                    `/public/tutorials/categories/query?offset="69"&outset="90"`
                )
                .expect(400)
                .end(done);
        });
